...
export function CPEditModal({ cp, candidate, mapCenter, mapZoom, onSave, onCancel, onDelete }: Props) {
  ...
  const handlePickerConfirm = (coords: [number, number][]) => {
    if (coords.length === 0) { setPickerOpen(false); return }
  ...
  const pickerCenter: [number, number] = form.coordinates[0] !== 0
    ? form.coordinates as [number, number]
    : mapCenter
  ...
  return (
    <>
          ...
          {field('CP緯度経度',
            <div>
              ...
              <button onClick={() => setPickerOpen(true)} style={{
                ...
                borderRadius: 6, cursor: 'pointer', fontSize: 13, color: '#2d6a4f', fontWeight: 600
              }}>
                🗺️ 地図上で位置を調整
              </button>
            </div>
      ...
      {pickerOpen && (
        <MapPositionPicker
          mode="point"
          initialCoords={form.coordinates[0] !== 0 ? [form.coordinates as [number, number]] : []}
